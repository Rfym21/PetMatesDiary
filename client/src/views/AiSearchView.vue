<template>
  <div class="page w-screen overflow-hidden">
    ...
    <div
      ...
      <van-uploader
        :class="`${(state.searchType && state.detail.image.length === 0) ? 'mx-auto' : ''} rounded-lg overflow-hidden`"
      ...
      <button class="send h-full rounded-lg overflow-hidden" v-if="state.searchType === false || state.imgUpload">
...
import { onMounted, reactive, watch, nextTick, ref } from 'vue'
